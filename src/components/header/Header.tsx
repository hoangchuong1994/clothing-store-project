  ...
  return (
    ...
    >
      <div className="flex h-16 items-center justify-between gap-2 px-4 sm:gap-4 sm:px-6 lg:px-8">
        {/* Logo */}
        <motion.div className="group relative shrink-0">
          <motion.h1
            whileHover={{ y: -2 }}
            transition={{ type: 'spring', stiffness: 300, damping: 20 }}
            className="text-foreground text-base font-black tracking-widest uppercase sm:text-lg"
          >
            {t('brand')}
          </motion.h1>
          {/* Animated Underline */}
          <motion.span
            ...
            style={{ originX: 0.5 }}
            className="absolute right-0 -bottom-1 left-0 h-1 rounded-full bg-gradient-to-r from-cyan-500 via-blue-500 to-cyan-500"
          />
        </motion.div>
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            transition={{ type: 'spring', stiffness: 300, damping: 20 }}
            className="text-foreground hover:bg-accent hover:text-accent-foreground focus-visible:ring-ring relative inline-flex items-center justify-center rounded-md p-2 transition-colors focus-visible:ring-2 focus-visible:outline-none"
